...
#appeareance of the diamond after every 5 seconds
...
while True:     
    ...
    for event in pygame.event.get(): #event speed of the game increases
        ...
        #checking timer and displaying diamonds in the display
        if event.type == TIMER:
            if len(diamonds) == 0:
                D = Diamond()
                diamonds.add(D)
    ...
    #giving 5 points for each diamond
    collected = pygame.sprite.spritecollide(P1, diamonds, False)
    for diamond in collected:
